...
class ChunkWriter(object):
  ...
  folder: folder in which to save the files
  ...
  def __init__(self, folder, size_limit = 128):
...
class FontDownloader(object):
  ...
  def __init__(self, output_folder: str):
    ...
    output_folder[-1] = "" if output_folder[-1] == "/"
    self.output_folder = output_folder

  def download_and_compress(self, retriver: FontRetriever, chunk_size = 125, **kwargs) -> None:
    """
  ...
  def is_fontfile(file: InMemoryFile) -> bool:
    ...
    try:
      ImageFont.truetype(file.content,50)
      return True
    except Exception e:
...
class Ingestor(object):
  ...
  config: Either an IngestionConfig instance or the path of the YAML configuration file

  """

  def __init__(self,config: typing.Union[IngestionConfig, str]) -> None:

    if isinstance(config, str):
      self.config = ConfigHandler.parse_ingestion_config_file(config)
    else:
      self.config = config
